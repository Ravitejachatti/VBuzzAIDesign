...
import CTCRangeFilter, { matchCTCRange } from "./ctcfilter";
import {
...
const PlacementReports = () => {
  ...
  // Companies & CTC options from reports
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const deptForCollege = departments.filter(d =>
      d.college === selectedCollege ||
      d.collegeId === selectedCollege ||
      (d.college && d.college._id === selectedCollege)
    );
  ...
  useEffect(() => {
    ...
    const progs = progIds.length
      ? programs.filter(p => progIds.includes(p._id))
      : programs.filter(
          p =>
            p.department === selectedDepartment ||
            p.departmentId === selectedDepartment ||
            (p.department && p.department._id === selectedDepartment)
        );
  ...
  const filteredReports = useMemo(
    () =>
      baseFilteredReports.filter(r => {
        ...
        const matchCTC =
          !selectedCTC ||
          placements.some(p => p.ctc && matchCTCRange(Number(p.ctc), selectedCTC));
  ...
  const stats = useMemo(() => {
    ...
    baseFilteredReports.forEach(r => {
      ...
      const total = off + on;
      if (total > 0) placedStudentsCount += 1;
  ...
  const downloadExcel = () => {
    const data = filteredReports.map(r => {
      const placements = [
        ...(r.offCampusPlacements || []),
        ...(r.onCampusPlacements || [])
      ];
      const placementDetails =
        placements
          .map(p => `${p.companyName || p.company || 'N/A'} (${p.status || 'N/A'})`)
          .join(', ') || 'No placements';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-3">
        <div>
          ...
          <input
            ...
            max={currentYear + 5}
          />
          {loading && <div className="mt-2 text-blue-500">Loading data...</div>}
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <button
                            ...
                            onClick={() => { setViewStudent(report); setModalOpen(true); }}
                          >
