...
const PlacementUpload = () => {
  ...
  const uploadData = async () => {
    ...
    try {
      const url = `${BASE_URL}/placement/upload-placements?universityName=${encodeURIComponent(universityName)}`;
      ...
      if (response.status === 200) {
        const { updatedRecords, failedRecords } = response.data;

        setUpdatedRecords(updatedRecords);
        setFailedRecords(failedRecords);
        setTotalUpdated(updatedRecords.length);
        setTotalFailed(failedRecords.length);

        setSuccess("Placement data uploaded successfully!");
      }
    } catch (err) {
      setError(
        ...
        <div className="mb-8 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <div className="flex flex-col md:flex-row md:items-center justify-between">
            <div className="mb-4 md:mb-0">
              ...
              <p className="text-gray-600">Use our template to ensure proper formatting</p>
            </div>
            <button
