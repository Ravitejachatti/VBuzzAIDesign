  ...
  useEffect(() => {
    const fetchApplicants = async () => {
      ...
      try {
        ...
        console.log("Enriched applicants data:", enriched);
      } catch (err) {
  ...
  >
    ...
    {jobs.map((job, index) => (
      <option key={job._id} value={job._id} className="text-xm text-gray-700 hover:bg-blue-100">
        {index + 1}. 
        <span className="font-bold text-lg"> Job: </span>{job.title}, 
        <span className="font-bold"> Company: </span>{job.company}, 
        <span className="font-bold"> Closing Date: </span>{new Date(job.closingDate).toLocaleDateString("en-US", {
          year: "numeric",
          month: "short",
          day: "numeric",
        })}
