...
const ResumeTemplate2 = () => {
  const studentData = JSON.parse(localStorage.getItem("studentData"))?.student || {};
  console.log("Student data in second template:", studentData);
    ...
    <Document>
      <Page style={styles.page}>
        ...
        <Text style={styles.header}>
          {studentData?.name?.toUpperCase() || "YOUR NAME"}
        </Text>
        <Text style={styles.contact}>
          {studentData?.contactInfo?.address?.city || "Hyderabad"},{" "}
          {studentData?.contactInfo?.address?.country || "India"} |{" "}
          {studentData?.email || "email@example.com"} |{" "}
          {studentData?.phone || studentData?.contactInfo?.phone || "123-456-7890"} |{" "}
          Reg No: {studentData?.registered_number || "XXXXXX"}
        </Text>
        ...
        <View style={styles.item}>
          ...
          <Text style={styles.bulletItem}>
            {studentData?.Bio || "Results-driven professional with expertise in multiple technical domains..."}
          </Text>
        </View>
        ...
        <View style={styles.item}>
          ...
          {studentData?.bachelors && (
            <View style={styles.item}>
              <Text style={styles.subHeader}>{studentData.bachelors.degree} in {studentData.bachelors.specialization}</Text>
              <Text style={styles.bulletItem}>
                {studentData.bachelors.institutionName}, {studentData.bachelors.university} | {studentData.bachelors.yearOfCompletion}
              </Text>
              <Text style={styles.bulletItem}>
                GPA: {studentData.bachelors.percentageOrCGPA} | {studentData.bachelors.notableAchievements}
              </Text>
            </View>
          )}
          ...
          {studentData?.masters && (
            <View style={styles.item}>
              <Text style={styles.subHeader}>{studentData.masters.degree} in {studentData.masters.specialization}</Text>
              <Text style={styles.bulletItem}>
                {studentData.masters.institutionName}, {studentData.masters.university} | {studentData.masters.yearOfCompletion}
              </Text>
              <Text style={styles.bulletItem}>
                GPA: {studentData.masters.percentageOrCGPA} | {studentData.masters.notableAchievements}
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.item}>
          ...
          <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
            {studentData?.skillsAndCompetencies?.technicalSkills?.map((skill, index) => (
        ...
        <View style={styles.item}>
          ...
          {studentData?.academicProjects?.map((project, index) => (
        ...
        <View style={styles.twoColumn}>
          ...
          <View style={styles.column}>
            ...
            <View style={styles.item}>
              ...
              {studentData?.skillsAndCompetencies?.languagesKnown?.map((lang, index) => (
