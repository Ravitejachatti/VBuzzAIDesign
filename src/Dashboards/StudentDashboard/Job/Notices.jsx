...
// Helpers to normalize values coming from backend
const toLower = (v) => (v ? String(v).toLowerCase() : "");
const titleCase = (s) => (s ? s.charAt(0).toUpperCase() + s.slice(1) : s);

// Priority styling (handles "high"/"High" etc.)
...
const ManageNotice = ({ setUnreadCount }) => {
  ...
  const [filter, setFilter] = useState("all"); // read-status filter: all | unread | read
  const [typeFilter, setTypeFilter] = useState("all"); // type filter: all | Job | Round | general | ...
  ...
  // Build unique type list from data; normalize case and fallback to "general"
  const noticeTypes = useMemo(() => {
    const set = new Set();
    notices.forEach((n) => {
      const t = n.type ? String(n.type).trim() : "general";
      // Normalize: e.g., "Job" and "job" become "Job"
      set.add(titleCase(toLower(t)));
    });
  ...
  const getFilteredNotices = () => {
    ...
    // type filter
    if (toLower(typeFilter) !== "all") {
      filtered = filtered.filter((n) => {
        const t = n.type ? String(n.type).trim() : "general";
        return toLower(t) === toLower(typeFilter);
      });
    }
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-3 mb-6">
        ...
        {/* Type Filter (dropdown for compact UI) */}
        <div className="flex items-center gap-2">
          <label className="text-sm text-gray-700">Type:</label>
          <select
            value={titleCase(typeFilter)}
            onChange={(e) => setTypeFilter(e.target.value)}
            className="px-3 py-2 rounded text-sm border bg-white"
          >
            {noticeTypes.map((t) => (
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500">
          No {filter} {toLower(typeFilter) !== "all" ? typeFilter : ""} notices available
        </p>
      ) : (
        <ul className="space-y-4">
          {filteredNotices.map((notice, index) => {
            const created = notice.createdAt ? new Date(notice.createdAt) : null;
            const expires = notice.expiryDate ? new Date(notice.expiryDate) : null;
            const typeChip = titleCase(toLower(notice.type || "general"));
            ...
            return (
              ...
              >
                <div className="flex">
                  <div className={`w-2 ${priorityBarClass(notice.priority)}`} />
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-600 mb-2">
                          ...
                          <span className="inline-flex items-center text-xs bg-gray-100 px-2 py-1 rounded border border-gray-200">
                            {typeChip}
                          </span>
                        </div>
                      ...
                      <div
                        className={`px-3 py-1 rounded-full border text-sm font-medium ${priorityBadgeClass(
                          notice.priority
                        )}`}
                      >
                        {titleCase(toLower(notice.priority || "info"))}
                      </div>
                    </div>
